...
class LafUtils {
  ...
  //static Color brillo = new Color( 255,255,255, 64);
    ...
    static Color getSombra() {
        return getColorAlfa(getColorTercio(LafLookAndFeel.getControlDarkShadow(), Color.black),
                64);
    }

    static Color getBrillo() {
        return getColorAlfa(getColorTercio(LafLookAndFeel.getControlHighlight(), Color.white),
                64);
    }
    ...
    static LafTheme iniCustomColors(LafTheme nt, Properties props) {
        String p1, p2, p3, s1, s2, s3, selection, background, w, b, opMenu, opFrame;

        selection = props.getProperty("Laflf.selection");
        background = props.getProperty("Laflf.background");

        p1 = props.getProperty("Laflf.p1");
        p2 = props.getProperty("Laflf.p2");
        p3 = props.getProperty("Laflf.p3");

        s1 = props.getProperty("Laflf.s1");
        s2 = props.getProperty("Laflf.s2");
        s3 = props.getProperty("Laflf.s3");

        w = props.getProperty("Laflf.w");
        b = props.getProperty("Laflf.b");

        opMenu = props.getProperty("Laflf.menuOpacity");
        opFrame = props.getProperty("Laflf.frameOpacity");
     ...
     */
    static LafTheme iniCustomColors(LafTheme nt, String selection, String background,
    ...
    static ImageIcon loadRes(String fich) {
        try {
            return new ImageIcon(Toolkit.getDefaultToolkit().createImage(readStream(LafLookAndFeel.class.getResourceAsStream(fich))));
    ...
    static int getMenuOpacity() {
        try {
            LafTheme th = (LafTheme) LafLookAndFeel.theme;
            return th.getMenuOpacity();
        } catch (Throwable ex) {
            return LafTheme.DEFAULT_MENU_OPACITY;
        }
    }
    ...
    static int getFrameOpacity() {
        try {
            ...
            return LafTheme.DEFAULT_FRAME_OPACITY;
        }
    }
